...
To use Lua you invoke the interpreter with the 'gopherlua:' key and pass it a YAML structure. The YAML structure is converted into Lua tables and set in the 'args' global variable where your script can access it. At the end of execution you pass data back to Goyamp in the 'results' global variable. This becomes the value of the gopherlua map which is substituted in the output.
...
2. The package.path variable is set to the value of environment variable LUA_PATH if present, 
otherwise the package.path variable is set to the default `./?.lua;./?.lc;<executable_directory>/lib/?.lua;<executable_directory>/lib/?.lc`. This means Lua will pick up files in `require()` calls from the `lib/` wherever goyamp is installed.   
...
Goyamp uses these global variables in the Lua interpreter:
