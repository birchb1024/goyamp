...
The `execute` builtin runs subprocesses and sends data to and from them. The syntax has two forms, the first takes a string argument: 
...
execute:
	...
	environment: < a map of strings containing an environment for the process> 
	directory: <a path string>
	response-type: "string"|"lines"|"json"|"yaml"
	request-type: "string"|"lines"|"json"|"yaml"
	request: <any yaml>
----

After execution, the stdout of the process is returned as the result without further processing. 
...
By default, the environment of the subprocess is inherited from the goyamp process. The environment variables for the command can be set with `environment`. 
When provided, a new empty environment is created, overriding the parent process environment. For example
...
To _add_ to the environment use `merge:`
...
When processing becomes more complex you may want to implent checks on input data. The `panic:` macro 
...
With the `-d` commnd-line option, a backtrace is also printed.
